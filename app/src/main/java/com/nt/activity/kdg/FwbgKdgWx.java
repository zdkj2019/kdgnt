...
public class FwbgKdgWx extends FrameActivity {
	...
	private String flag, zbh, msgStr, zdh, zbh_xj, xzpj_str, sbxhbm, sblx,
			sbxh, bzr, lxdh, ywlx2, ds, ywhy_bm, ewm = "", hpbm = "", sbxlhs,
			wxts, rgfy;
	...
	protected void initListeners() {
					...
					if (rg_0.getCheckedRadioButtonId() == -1) {
						...
						if (rg_0.getCheckedRadioButtonId() == R.id.rb_1) {
							...
							if (cb_xzpj.isChecked()) {
								...
								dialogShowMessage("本次服务未更换配件，是否继续提交？",
							...
							if ("".equals(gzbm_3_list.get(
									spinner_gzxl.getSelectedItemPosition())
									.get("id"))) {
							...
							if (Integer.parseInt(et_wxts.getText().toString()
									.trim()) == 0) {
								toastShowMessage("请录入维修台数！");
								return;
							}
		...
		rg_0.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
	...
	protected void getWebService(String s) {
			...
			try {
				...
				if (Integer.parseInt(flag) > 0) {
					...
					jsonObject = callWebserviceImp
							.getWebServerInfo("_PAD_KDG_FWBG_WGQKXL", "",
		...
		if (s.equals("submit")) {// 提交
			try {
				...
				str += clgcs;
				str += "*PAM*";
				str += et_wxts.getText().toString().trim();
				str += "*PAM*";
				str += sbxlhs;
				str += "*PAM*";
	...
	private Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case Constant.NUM_6:
				...
				adapter = new SimpleAdapter(FwbgKdgWx.this, data_smyy,
						R.layout.spinner_item, from, to);
				spinner_smyy.setAdapter(adapter);

				adapter = new SimpleAdapter(FwbgKdgWx.this, data_wgqk,
						R.layout.spinner_item, from, to);
				spinner_wgqk.setAdapter(adapter);

				adapter = new SimpleAdapter(FwbgKdgWx.this, data_sblx,
						R.layout.spinner_item, from, to);
				spinner_sblx.setAdapter(adapter);

				adapter = new SimpleAdapter(FwbgKdgWx.this, data_lxdh,
						R.layout.spinner_item, from, to);
			...
			case Constant.NUM_8:
				...
				SimpleAdapter adapter1 = new SimpleAdapter(FwbgKdgWx.this,
